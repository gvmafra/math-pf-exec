...
const genStageFive = (): Stage => ({
  levels: initializeLevels(
    ...
    concatArrays<GameCanvas>([
      [
        genGridCanvas({ rows: 2, columns: 1 }, { rows: [2], columns: [1, 2, 3] }), // challenge canvas 1
        genGridCanvas({ rows: 2, columns: 1 }, { rows: [2], columns: [1, 2, 3] }), // challenge canvas 2
        genGridCanvas({ rows: 1, columns: 2 }, { rows: [1, 2, 3], columns: [2] }), // challenge canvas 3
        genGridCanvas({ rows: 1, columns: 2 }, { rows: [1, 2, 3], columns: [2] }), // challenge canvas 4
      ],
      initializeLevelChallenges([
